...
class Interface(object):
    ...
    def joint_control(self, body, joints = None, values = None, mode = 'position', force = None):
        """
        ...
        if mode == 'position':
            if force:
                ...
                for i in range(len(joints)):
                    p.setJointMotorControl2(bodyIndex = body,
                                            jointIndex = joints[i],
                                            controlMode = p.POSITION_CONTROL,
    ...
    def keep_pose(self, body):
        length = self.vel.array.shape[0]
        joints = self.movable_g_joints
        values = [0.0] * len(self.movable_g_joints)
        for i in range(7675 - length):
            self.joint_control(body, joints, values, 'position', 15)
            p.stepSimulation(physicsClientId = self.client_id)
            if 'vel' in self.__dict__:
                self.vel.concatenate(np.array([self.get_joint_velocities(body)]))
