...
findPath=function(startConfig,goalConfigs,cnt)
    ...
    local jointLimitsL={-360*math.pi/180}
    local jointLimitsH={360*math.pi/180}
    ...
    for i=1,#jh,1 do
        ...
        jSpaces[#jSpaces+1]=simExtOMPL_createStateSpace('j_space'..i,sim_ompl_statespacetype_joint_position,jh[i],{jointLimitsL[1]},{jointLimitsH[1]},proj)
    end
...
ikGroup=simGetIkGroupHandle('UR5')
-- Created by IntelliJ IDEA.
target=simGetObjectHandle('target1')
...
local m=simGetObjectMatrix(target,-1)
local configs=findSeveralCollisionFreeConfigs(m,300,5)
if configs then
    --simAddStatusbarMessage(#configs)

end
--simAddStatusbarMessage('over')
