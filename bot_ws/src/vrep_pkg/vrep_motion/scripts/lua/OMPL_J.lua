...
savePath=function(filename,path,lengths)
    simWriteCustomDataBlock(baxterBaseHandle,filename..'.pathData1',simPackFloatTable(path))
    simWriteCustomDataBlock(baxterBaseHandle,filename..'.pathLength1',simPackFloatTable(lengths))
end

loadPath=function(filename)
    path=simReadCustomDataBlock(baxterBaseHandle,filename..'.pathData1')
    if (not path) then return nil end
    path=simUnpackFloatTable(path)

    lengths=simReadCustomDataBlock(baxterBaseHandle,filename..'.pathLength1')
...
collisionPairs={simGetCollectionHandle('Baxter_leftArm'),sim_handle_all}
maxVel=3
maxAccel=1
maxJerk=8000
forbidLevel=0
...
ikSteps=20
maxOMPLCalculationTime=5-- for one calculation. Higher is better, but takes more time
OMPLAlgo=sim_ompl_algorithm_BKPIECE1 -- the OMPL algorithm to use
numberOfOMPLCalculationsPasses=6-- the number of OMPL calculation runs for a same goal config. The more, the better results, but slower

-- Uncomment following to force not using stored motion data,
-- and to regenerate the motion data:
--simWriteCustomDataBlock(baxterBaseHandle,'',nil)
